...
for c in cases:
    ...
    for n in range(0,n_submits):
        ...
        if (os.path.isfile(caseroot+'/'+c+'/case.run')):
       	     # Modified by Ethan and Ming 180425
	    # if ('itasca' in options.machine):
	    #	with open(caseroot+'/'+c+'/case.run','r') as tmpin:
            #	    buf = tmpin.readlines()
            #	with open(caseroot+'/'+c+'/case.run','w') as input:
            #	    for line in buf:
            #	        if line=='#PBS  -V \n':
	    ...
	    # Modified by Ming and Ethan 180428
           # if ('itasca' in options.machine):
	   # 	with open(caseroot+'/'+c+'/.case.run','r') as tmpin:
	   # 	    buf = tmpin.readlines()
           # 	with open(caseroot+'/'+c+'/case.run','w') as input:
        ...
        for s in input:
		    ...
		    # Modified by Ming and Ethan
                    if ('itasca' in options.machine):
                        nodes=int(numpy.ceil(float(options.np)/8.0))
                        output.write('#PBS  -l walltime='+str(options.walltime)+':00:00,nodes='+str(nodes)+':ppn=8\n')
		    elif ('mesabi' in options.machine):
			nodes=int(numpy.ceil(float(options.np)/24.0))
	                output.write('#PBS  -l walltime='+str(options.walltime)+':00:00,nodes='+str(nodes)+':ppn=24\n')
                    else:
		    #End of change
	...
	if ('mesabi' in options.machine):
	    ...
	    #output.write('module load ompi/3.0.0/intel-2018-release\n')
	...
	# Ethan 2018-05-16
	if ('itasca' in options.machine or 'mesabi' in options.machine):
        	output.write("./xmlchange -id PIO_TYPENAME -val netcdf\n")
	#if ('mesabi' in options.machine):
		#output.write("./xmlchange -id USE_MOAB -val TRUE\n")
	...
	if ('ad_spinup' in c and n == (n_submits-1)):
            if (options.bgc):
		# Ethan 2018-04-30 removed / after run
