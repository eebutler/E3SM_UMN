...
#---------------------------------------------------------------------------------
        ...
        for c in case_list:
            ...
            if ((sitenum % 32) == 0):
                ...
                for s in input:
                    if ("perl" in s or "python" in s):
                        ...
                        if (options.debug):
                            timestr = '00:30:00'
			if ("itasca" in options.machine or 'mesabi' in options.machine):
                    	    output.write("#!/bin/sh -l\n")
                	else:
                    	    output.write("#!/bin/csh -f\n")
                        if (mysubmit_type == 'qsub'):
			    # EEB
			    if ('itasca' in options.machine):
                        	nodes=int(numpy.ceil(float(options.np)/8.0))
                        	output.write('#PBS  -l walltime='+str(options.walltime)+':00:00,nodes='+str(nodes)+':ppn=8\n')
                    	    elif ('mesabi' in options.machine):
                        	nodes=int(numpy.ceil(float(options.np)/24.0))
				output.write('#PBS  -l walltime='+str(options.walltime)+':00:00,nodes='+str(nodes)+':ppn='+options.np+'\n')
                    	    else:			
			...
			elif ("itasca" in options.machine or 'mesabi' in options.machine):
			    output.write("\n")
                        else:
		...
		# EEB
		if ("itasca" in options.machine):
		    output.write('module load python-epd/1.5.2\n')
            	    output.write('module load perl/5.14.2\n')
            	    output.write('module load libxml2/10.19.2017\n')
            	    output.write('module load cmake\n')
            	    output.write('module load nco\n')
            	    output.write('module load netcdf/fortran-4.2.intel\n')
            	    output.write('module load ompi/1.8.5/intel-2015-update3\n')
		if ('mesabi' in options.machine):
	    	    output.write('module load python2\n')
            	    output.write('module load perl/5.14.2\n')
            	    output.write('module load libxml2/2.9.10\n')
            	    output.write('module load cmake\n')
            	    output.write('module load nco/4.8.1\n')
            	    #output.write('module unload netcdf\n')
		    #output.write('module load netcdf/4.62-intel-2019-update1-serial\n')
		    #output.write('module load netcdf/fortran-4.4.4-intel-2019-update1-serial\n')
		    #output.write('module load netcdf/fortran-4.4.4-intel-2018-release-serial\n') #NCO 4.8.1 module loads its own compatible netcdf-fortran
            	    #output.write('module load impi\n')
            	    #output.write('module load ompi/4.0.0/intel-2019-update1-centos7\n') # NCO 4.8.1 also beings in a compatible ompi
		    #output.write('module load impi/2018/release_singlethread_debug\n') 
            else:
